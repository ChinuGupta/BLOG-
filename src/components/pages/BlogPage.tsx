...
//           <input type="search" name="searchblogs" className="w-1/2 rounded-xs border p-1" placeholder="Search Blogs" onChange={handleSearch} value={searchedblog} onBlur={() => setTimeout(() => setSuggestions([]), 100)} />
//           {suggestions.length > 0 && (
//             <ul className="absolute z-10 mt-1 bg-white border w-1/2 rounded shadow-md">
//               {suggestions.map((suggestion, index) => (
//                 <li
//                   key={index}
//                   className="p-2 hover:bg-blue-100 cursor-pointer"
//                   onClick={() => {
//                     setSearchedBlog(suggestion);
//                     setSuggestions([]);
//                   }}
//                 >
//                   {suggestion}
...
export default function BlogPage() {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const blogTitles = bloglist.map((blog) => blog.title);
    const results = fuzzy.filter(e.target.value, blogTitles);
    const matchedTitles = results.map((res) => res.original);
    setSuggestions(matchedTitles);
  };

  const handleSubmitSearch = (value?: string) => {
    const searchValue = value || searchedblog;
    const blogTitles = bloglist.map((blog) => blog.title);
    const results = fuzzy.filter(searchValue, blogTitles);
    const matchedTitles = results.map((res) => res.original);
  ...
  const fetchbloglist = () => dispatch(fetchBlogs());
  const totalPost = Math.ceil(filteredblog.length / blogperpage);
  const paginatedblog = filteredblog.slice(page * blogperpage - blogperpage, page * blogperpage);
  ...
  return (
    <div className="min-h-[36rem]">
      ...
      <form
        ...
        className="flex justify-center items-center mb-10 w-full"
      >
        <div className="relative w-1/4">
          ...
          {suggestions.length > 0 && (
            <ul className="absolute z-10 mt-1 bg-white border w-full rounded shadow-md max-h-60 overflow-y-auto">
              {suggestions.map((suggestion, index) => (
                <li
                  key={index}
                  className="p-2 hover:bg-blue-100 cursor-pointer"
        ...
        <select
          className="mx-6 p-2 border rounded-md bg-blue-500 text-white cursor-pointer"
          onChange={sortbydate}
        >
          <option value="asc">Oldest First</option>
          <option value="desc">Newest First</option>
      ...
      {paginatedblog.length > 0 && (
        <div className="flex items-center justify-center gap-4 p-4 bg-gray-100 rounded-lg shadow-md mt-20">
          <button
            onClick={handleprev}
            disabled={page === 1}
            className={`p-2 w-40 ${page === 1 ? 'bg-gray-200' : 'bg-[#3B82F6]'}`}
          >
            ◀ Prev
          </button>
          ...
          >
            {
              [...Array(Math.ceil(filteredblog.length / blogperpage))].map((_, i) => (
          ...
          <button
            onClick={handlenext}
            disabled={page === totalPost}
            className={`p-2 w-40 ${page === totalPost ? 'bg-gray-200' : 'bg-[#3B82F6]'}`}
          >
            Next ▶
          </button>
        </div>
